...
__urdlllocal ur_result_t UR_APICALL urEnqueueUSMMemAdvice(
    ur_queue_handle_t hQueue, ///< [in] handle of the queue object
    const void *pMem,         ///< [in] pointer to the USM memory object
...
__urdlllocal ur_result_t UR_APICALL urMemFree(
    ur_context_handle_t hContext, ///< [in] handle of the context object
...
__urdlllocal ur_result_t UR_APICALL urMemGetMemAllocInfo(
    ur_context_handle_t hContext, ///< [in] handle of the context object
...
__urdlllocal ur_result_t UR_APICALL urDevicePartition(
    ...
    // forward to device-platform
    result = pfnPartition(hDevice, Properties, NumDevices, phSubDevices,
                          pNumDevicesRet);
...
__urdlllocal ur_result_t UR_APICALL urInit(
    ...
    for (auto &platform : context->platforms) {
        if (platform.initStatus != UR_RESULT_SUCCESS)
            continue;
        platform.initStatus =
            platform.dditable.ur.Global.pfnInit(platform_flags, device_flags);
        if (platform.initStatus == UR_RESULT_SUCCESS)
